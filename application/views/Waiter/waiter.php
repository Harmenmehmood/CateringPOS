                ...
                <div class="card bg-dark w-100 mt-1">
                    ...
                    <table class="table_order_total bg-dark mt-1 text-white rounded w-100" cellpadding="1">
                        ...
                        <tr>
                            ...
                            <th class="text-center p-2" width="50%"><Button class="form-control btn btn-danger" id="btn_cancle" style="width: 100%; height: 100%;" class="btn-sm btn-danger">Reset Order</Button></th>
                        </tr>
    ...
    $(document).ready(function() {
        ...
        setInterval(() => {
            <?php foreach ($tables_data as $table) {
                ...
                if ($this->session->userdata('t-' . $table['table_num']) == "cooking" || $this->session->userdata('t-' . $table['table_num']) == "ready") { ?>

                    $.ajax({
                        url: "<?php echo site_url("Waiter/Waiter/check_status"); ?>",
                        type: "POST",
                        data: {
                            table_num: '<?php echo $table['table_num'] ?>'
                        },
                        cache: false,
                        success: function(res) {
                            if (res === '1') {
                                swal.fire({
                                    title: "Table #<?php echo $table['table_num'] ?>\nOrder Is Ready!.",
                                    text: '',
                                    icon: "success",
